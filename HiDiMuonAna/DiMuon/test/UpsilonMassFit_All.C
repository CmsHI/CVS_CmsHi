...
void UpsilonMassFit_All(int isData = 2, int iSpec = 1)
{
 ...
 //==============================================Define AccEff Stuff here===========================================
  ...
  for(int ifile =0; ifile<=5; ifile++){
    ...
    double GenmuPosPx, GenmuPosPy, GenmuPosPz,  GenmuPosEta, GenmuPosPt, GenmuPosP;
    double GenmuNegPx, GenmuNegPy, GenmuNegPz,  GenmuNegEta, GenmuNegPt, GenmuNegP;
	...
	cout<<" Mass "<< GenJpsiMass<< " pT "<< GenJpsiPt << " Y " <<GenJpsiRap<<endl; 
      ...
      GenmuPosP = TMath::Sqrt(GenmuPosPx*GenmuPosPx + GenmuPosPy*GenmuPosPy+ GenmuPosPz*GenmuPosPz); 
      GenmuNegPt= TMath::Sqrt(GenmuNegPx*GenmuNegPx + GenmuNegPy*GenmuNegPy); 
      GenmuNegP = TMath::Sqrt(GenmuNegPx*GenmuNegPx + GenmuNegPy*GenmuNegPy +GenmuNegPz*GenmuNegPz); 
      
      diMuonsInvMass_Gen->Fill(GenJpsiMass);
      diMuonsPt_Gen->Fill(GenJpsiPt);
 
      if(IsAccept(GenmuPosPt, GenmuPosEta, GenmuPosP)) {GenPosIn=1;}
      if(IsAccept(GenmuNegPt, GenmuNegEta, GenmuNegP)) {GenNegIn=1;}
	...
	cout<<" Mass "<< JpsiMass<< " pT "<< JpsiPt << " Y " <<JpsiRap<<"  "<<JpsiVprob<<" charge "<<JpsiCharge<<endl; 
      ...
      if(IsAccept(muPosPt, muPosEta, muPosP)){PosIn=1;}
      if(IsAccept(muNegPt, muNegEta, muNegP)){NegIn=1;}
	 ...
	 && muPos_nValidMuHits > 6){PosPass=1;}	  
      ...
      //cout<<" muNeg_matches "<< muNeg_matches<< "   "<<  muPos_matches<<endl;
      
      if( (PosIn==1 && NegIn==1 )  &&   (PosPass==1 && NegPass==1)){AllCut=1;}
      //only muon quality cuts
      
      //if(((PosIn==1 && NegIn==1 ))){AllCut=1;}
...
//if(iSpec == 2) if( (AllCut) && (JpsiPt > 6.5 && JpsiPt< 30.0) && (JpsiRap>pt_bound[ih] && JpsiRap<=pt_bound[ih+1])) {diMuonsInvMass_RecA[ifile][ih]->Fill(JpsiMass);}
  ...
  MassJPsi = 9.46; WidthJPsi = 0.054;
  mass_low = 8.0; mass_high = 12.0;  // Fit ranges
  mlow = 8.0; mhigh = 12.0;   // Draw ranges
  nrebin = 1; isLog = 0; isFit = 1; 
   
  //New Fit Functon
  //TF1 *f1 = new TF1("f1","[0]*TMath::Gaus(x,[1],[2],1) + TMath::Pol2(x,[3],[4])",8.0,12.0);
  ...
  legend_GP->AddEntry(Eff_JPsi,"Pythia Gun in MB HI Data ", "P");
