...
vector<double> fitPeak_old(RooRealVar x, Int_t whichBkg=0,const Int_t nPeak=1, TH1* hist=0, RooDataSet* dataSet, vector<Double_t> fMean, vector<Double_t> dfMean,vector<Double_t> fSigma, vector<Double_t> dfSigma,Double_t fbkg=0.5)

{
	...
	if(hist)
	{
	  fitRlt=model->fitTo(*dataHist,Extended(kTRUE),Minos(kTRUE),Save(kTRUE),NumCPU(7),SumW2Error(kTRUE));
	  //  fitRlt=model->fitTo(*dataHist,Extended(kTRUE),Minos(kTRUE),Save(kTRUE),NumCPU(7));

	} 
	else if(dataSet)
	{   
	  fitRlt=model->fitTo(*dataSet,Extended(kTRUE),Minos(kTRUE),Save(kTRUE),NumCPU(7),SumW2Error(kTRUE));
	...
	if(getSB)
	{
		...
		RooAbsReal* igPeak=peak[0]->createIntegral(x,NormSet(x),Range("sig0"));  //prob in massrange -2s to +2s
		RooAbsReal* igBkg=bkg->createIntegral(x,NormSet(x),Range("sig0"));
		double ratioPeak=(double)areaPeak[0]->getVal()*(double)igPeak->getVal(); //signal events in the mass range igPeak
		...
		num0=(double)nEntries*(double)frac0->getVal(); // events in -2s to 2s (Sig+Bkg)
		num1=(double)nEntries*((double)frac1->getVal()+(double)frac2->getVal()); //  events outside  -4s to +4s 
		num2=areaPeak[0]->getError();  
		...
		cout<<"========fracIntgr: "<<num2<<"=============="<<endl;

		sigBkg.push_back(Mean);
		sigBkg.push_back(Sigma);

		sigBkg.push_back(ratioSB0);  // (ratiopeak=signal events in the mass range igPeak)/ ratiobkg
		sigBkg.push_back(ratioSB1);  // Significance

		sigBkg.push_back(num0);   // events in -2s to 2s (Sig+Bkg)
		sigBkg.push_back(num1);   // events outside  -4s to +4s 

		sigBkg.push_back((double)areaPeak[0]->getVal()); // No.of Signals cal by fitting the mass dist
		sigBkg.push_back((double)areaBkg->getVal());

		sigBkg.push_back(ratioPeak);
		sigBkg.push_back(ratioBkg);
		sigBkg.push_back(num2);
