...
// Double Muon Trigger
  ...
  HLTbitNames_.push_back( "HLT_HIL1DoubleMuOpen" );
...
// Double Muon Trigger
  mapTriggerToLastFilter_["HLT_HIL1DoubleMuOpen"]                  = "hltHIDoubleMuLevel1PathL1OpenFiltered";
...
JPsiAnalyzerPAT::beginJob()
{
  ...
  if (_writeTree) {
    ...
    tree_->Branch("muNegP", "TLorentzVector", &muNegP);
     ...
     tree_->Branch("muNegPz",    &muNegPz,   "muNegPz/D");
		...
		//-----------------------------------------------------
		 ...
		 tree_->Branch("mu_Pos_SegMatch",&mu_Pos_SegMatch,"mu_Pos_SegMatch/I");
		 ...
		 tree_->Branch("muNeg_oneStationAngTight",&muNeg_oneStationAngTight,"muNeg_oneStationAngTight/I");
	 	 tree_->Branch("mu_Neg_SegMatch",&mu_Neg_SegMatch,"mu_Neg_SegMatch/I");
    ...
    //tree->Branch("HLT_HIL1DoubleMuOpen", &HLT_HIL1DoubleMuOpen);
...
void JPsiAnalyzerPAT::fillTreeAndDS(unsigned int theCat, const pat::CompositeCandidate* aCand, const edm::Event& iEvent){
  ...
  //    if (!(isMuonInAccept(muon1) && isMuonInAccept(muon2))) return;
  ...
  muNegP->SetPxPyPzE(f_muNegPx, f_muNegPy, f_muNegPz, enMuNeg);
	...
	if(muonPos->isTrackerMuon())
	{
		...
		mu_Pos_SegMatch = muonPos->numberOfMatches(reco::Muon::SegmentArbitration);
		
		cout<<"Pos_Seg_match = "<<mu_Pos_SegMatch<<endl; 
		...
		if(muonPos->isGlobalMuon())
		{
			...
			cout<<"Pos_Validhits = "<<muPos_nValidMuHits<<endl;
  ...
  //2.Negative Muobn
	if(muonNeg->isTrackerMuon())
	{
		...
		mu_Neg_SegMatch = muonNeg->numberOfMatches(reco::Muon::SegmentArbitration);

		cout<<"Neg_Seg_match = "<<mu_Neg_SegMatch<<endl; 
		...
		if(muonNeg->isGlobalMuon())
		{
			...
			cout<<"Neg_Validhits = "<<muNeg_nValidMuHits<<endl;
...
void JPsiAnalyzerPAT::makeCuts(int sign) {
  ...
  if (collAll.isValid()) {
	...
	// apply acceptance cut //// Shukla

	if (isMuonInAccept(muon1) && isMuonInAccept(muon2))  {
...
// *USE* muon kinematical cuts (eta dependent momentum / pT cuts )Old cut used for pp
  //return (fabs(aMuon->eta()) < 2.4 &&
...
void JPsiAnalyzerPAT::resetDSVariables(){
...
//
	...
	// All the other Paths match by last filter:
        // double muon trigger:
        if ( triggerName == "HLT_HIL1DoubleMuOpen" && pass1 == true && pass2 == true ) mapTriggerNameToIntFired_[triggerName] = 1;
