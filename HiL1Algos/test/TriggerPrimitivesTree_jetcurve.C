				      ...
				      enum SUBTRACT_ALGORITHM algorithm)
{
  ...
  const int nbins = 20; //number of jet bins from 0 to 100
  const int etabins = 45; //number of eta bins from -4.5 to +4.5
 
  stringstream name[etabins];
  stringstream name2[etabins];
  stringstream name3[etabins];
  TH1D* jet_curve[etabins];
  TH1I* total_in_bin[etabins];

  for(int i = 0; i < etabins; i++)
  {
    name[i] << "Jet Turn-On Curve, threshold:" << threshold << " compressed Et. Eta bin:" << i;
    name2[i] << "total_in_bin_" << i;
    name3[i] << "jet_curve_"<<i;
    jet_curve[i] = new TH1D(name3[i].str().c_str(), name[i].str().c_str(),
			    nbins,0,100);
    total_in_bin[i] = new TH1I(name2[i].str().c_str(),name2[i].str().c_str(),
			       nbins,0,100);
  }
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    int jetetabin = (int) ((realjeteta + 4.5) * (etabins/9));
  ...
  for(int i = 0; i < etabins; i++)
  {
    ...
    jet_curve[i]->Draw("E,same");
  //jet_curve->ProfileX()->Draw();
  }
