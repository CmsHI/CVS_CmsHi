...

{
  ...
  TH2D* jetDelta_ = new TH2D("jetDelta","",100,0,6,150,-50,100);
  ...
  for(event.toBegin(); !event.atEnd(); ++event, ++iEvent){
    ...
    for(unsigned i=0; i<jets->size(); ++i){
	 ...
	 jetDelta_->Fill(deltaR((*jets)[i].eta(),(*jets)[i].phi(),(*jets)[i].genJet()->eta(),(*jets)[i].genJet()->phi()),(*jets)[i].pt() - (*jets)[i].genJet()->pt());
