...
/** \class HTupleManager  Ganzhur/HafHistogram/src/HTupleManager.cc
 *
 * Description:
 ...
 *
 * \version $Id: HTupleManager.cc,v 1.3 2009/11/19 09:40:31 ganzhur Exp $
 *
 */
...
HMassHistogram* HTupleManager::MassHistogram(const Text_t *hname,const Text_t *htitle,Int_t nbin,Axis_t lowX,Axis_t highX) 
{
    TDirectory* olddir= gDirectory;
    checkFile();
    HMassHistogram* histp= (HMassHistogram*) getHistByName( htitle );
    if( histp == 0 ) {
	histp=new HMassHistogram( &*hname, htitle, nbin, lowX, highX );
	hlist->Add(histp);
    }
    olddir->cd();
    return histp;
...
HEnergyHistogram* HTupleManager::EnergyHistogram(const Text_t *hname,const Text_t *htitle,Int_t nbin,Axis_t lowX,Axis_t highX) 
{
    TDirectory* olddir= gDirectory;
    checkFile();
    HEnergyHistogram* histp= (HEnergyHistogram*) getHistByName( htitle );
    if( histp == 0 ) {
	histp=new HEnergyHistogram( &*hname, htitle, nbin, lowX, highX );
	hlist->Add(histp);
    }
    olddir->cd();
    return histp;
...
HMomentumHistogram* HTupleManager::MomentumHistogram(const Text_t *hname,const Text_t *htitle,Int_t nbin,Axis_t lowX,Axis_t highX) 
{
    TDirectory* olddir= gDirectory;
    checkFile();
    HMomentumHistogram* histp= (HMomentumHistogram*) getHistByName( htitle );
    if( histp == 0 ) {
	histp=new HMomentumHistogram( &*hname, htitle, nbin, lowX, highX );
	hlist->Add(histp);
    }
    olddir->cd();
    return histp;
...
HEoverPHistogram* HTupleManager::EoverPHistogram(const Text_t *hname,const Text_t *htitle,Int_t nbinX,Axis_t lowX,Axis_t highX
	,Int_t nbinY,Axis_t lowY,Axis_t highY)
{
    TDirectory* olddir= gDirectory;
    checkFile();
    HEoverPHistogram* histp= (HEoverPHistogram*) getHistByName( htitle );
    if( histp == 0 ) {
	histp=new HEoverPHistogram( &*hname, htitle,nbinX, lowX, highX,nbinY, lowY, highY );
	hlist->Add(histp);
    }
    olddir->cd();
    return histp;
...
HMoverPHistogram* HTupleManager::MoverPHistogram(const Text_t *hname,const Text_t *htitle,Int_t nbinX,Axis_t lowX,Axis_t highX
	,Int_t nbinY,Axis_t lowY,Axis_t highY)
{
    TDirectory* olddir= gDirectory;
    checkFile();
    HMoverPHistogram* histp= (HMoverPHistogram*) getHistByName( htitle );
    if( histp == 0 ) {
	histp=new HMoverPHistogram( &*hname, htitle,nbinX, lowX, highX,nbinY, lowY, highY );
	hlist->Add(histp);
    }
    olddir->cd();
    return histp;
