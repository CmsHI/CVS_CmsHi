                   ...
                   )
{
   ...
   for (int ib=0; ib<vcutCent.size(); ++ib) {
      ...
      vana[ib]->cutBkgDPhi       = vana[ib]->cutSigAllPho&&Form("abs(inclJetEta)<1.6&&inclJetPt>%.3f&&acos(cos(photonPhi-inclJetPhi))>0.7 && acos(cos(photonPhi-inclJetPhi))<3.14159/2.",minJet);
      ...
      vana[ib]->cutSShapeDPhi    = vana[ib]->cutSShapeAllPho&&Form("abs(inclJetEta)<1.6&&inclJetPt>%.3f&&acos(cos(photonPhi-inclJetPhi))>0.7 && acos(cos(photonPhi-inclJetPhi))<3.14159/2.",minJet);
      if (doMixBkg) {
      ...
      if (dataType==0) {
         ...
         vana[ib]->MakeHistograms(Form("abs(inclJetEta)<1.6&&inclJetPt>%.03f && acos(cos(photonPhi-inclJetPhi))>%f",minJet,sigDPhi),nxbins,xmin,xmax);
      } else {
...
//         vana[ib]->SetJetWeights("inclJetPt");
         vana[ib]->MakeHistograms(Form("abs(inclJetEta)<1.6&&inclJetPt>%.03f && acos(cos(photonPhi-inclJetPhi))>%f && sigmaIetaIeta<0.01",minJet,sigDPhi),nxbins,xmin,xmax);
      }
...
//---------------------------------------------------------------------
void plotInclPtRatioSignal_AllCent4_wSummary(
...
//   getHistograms(vanapp7, vcutCentPp,"anaEvtSel",isolScheme,normMode,"../output-pp-photon-7TeV-v4_v24_akPu3PF.root","(1==1)",3,1,subDPhiSide,subSShapeSide,minPhoton,minJet,sigDPhi);
   ...
   mcweight = "((samplePtHat==30&&pthat<50)||(samplePtHat==50))*weight*sampleWeight";
   vector<SignalCorrector*> vanapyz2;
//   getHistograms(vanapyz2, vcutCentPp,"offlSel&&genCalIsoDR04<5&&abs(refPhoFlavor)<=22",isolScheme,normMode,"../output-qcdpho30and50hiSignalv2_v30_xsec_akPu3PF.root",mcweight,10,1,subDPhiSide,0,minPhoton,minJet,sigDPhi);
   getHistograms(vanapyz2, vcutCentPp,"offlSel",isolScheme,normMode,"../output-qcdpho30and50hiSignalv2_v30_xsec_akPu3PF.root",mcweight,10,1,subDPhiSide,0,minPhoton,minJet,sigDPhi);
...
//   plotHistograms(vanapp7[0],cbin,3,1,drawCheck,"sameE");
   ...
   c1->Print(Form("%s/Photonv7_v29_akPu3PF_isolPho_gamma%.0fjet%.0fdphiSig%.0f_InclPtRatio_all_cent4_subDPhi%dSS%d_Isol%d_Norm%d_drawChk%d_log%d.gif",outdir.Data(),minPhoton,minJet,sigDPhi*1000,subDPhiSide,subSShapeSide,isolScheme,normMode,drawCheck,log));
   c1->Print(Form("%s/Photonv7_v29_akPu3PF_isolPho_gamma%.0fjet%.0fdphiSig%.0f_InclPtRatio_all_cent4_subDPhi%dSS%d_Isol%d_Norm%d_drawChk%d_log%d.pdf",outdir.Data(),minPhoton,minJet,sigDPhi*1000,subDPhiSide,subSShapeSide,isolScheme,normMode,drawCheck,log));
...
//      leg->AddEntry(gpyd6t,"PYTHIA 2.76 TeV D6T","p");
      ...
      c2->Print(Form("%s/Photonv7_v29_akPu3PF_isolPho_InclPtRatio_SubDPhi%dSS%d_gamma%.0fjet%.0fdphiSig%.0f_vs_Npart_Isol%d_Norm%d_anaMode%d_drawChk%d.gif",outdir.Data(),subDPhiSide,subSShapeSide,minPhoton,minJet,sigDPhi*1000,isolScheme,normMode,summaryMode,drawCheck));
      c2->Print(Form("%s/Photonv7_v29_akPu3PF_isolPho_InclPtRatio_SubDPhi%dSS%d_gamma%.0fjet%.0fdphiSig%.0f_vs_Npart_Isol%d_Norm%d_anaMode%d_drawChk%d.pdf",outdir.Data(),subDPhiSide,subSShapeSide,minPhoton,minJet,sigDPhi*1000,isolScheme,normMode,summaryMode,drawCheck));
   }
}
