...
)
{
   ...
   for (int ib=0; ib<nBin; ++ib) {
      ...
      vana[ib]->cutSShape= Form("jetEt>%.3f&&acos(cos(photonPhi-jetPhi))>0.7 && sigmaIetaIeta>0.011 && sigmaIetaIeta<0.017",minJet);
      ...
      vana[ib]->Extrapolate(0.7,true);
      vana[ib]->ExtrapolateDPhiHist(0.7);
      vana[ib]->SubtractBkg();
      vana[ib]->Normalize(1);
   }
