...
)
{
   ...
   {
      int imin = vana[ib]->rSubtracted.hExtrapNorm->FindBin(ajCut);
      int imax = vana[ib]->rSubtracted.hExtrapNorm->FindBin(10);
      float y=vana[ib]->rSubtracted.hExtrapNorm->Integral(imin,imax);
      float errYL = y/sqrt(vana[ib]->rSubtracted.nExtrap);
      float errYH = y/sqrt(vana[ib]->rSubtracted.nExtrap);
                            ...
                            )
{
   ...
   hTmp->SetYTitle(Form("R(x_{JG}> %.1f)",ajCut));
   hTmp->GetXaxis()->CenterTitle();
   hTmp->GetYaxis()->CenterTitle();
   hTmp->GetYaxis()->SetTitleOffset(1.4);
   hTmp->GetYaxis()->SetTitleSize(0.05);
   float ymin=0,ymax=1.7; // 35, 2, 0.4
   hTmp->SetAxisRange(ymin,ymax,"Y");
   TCanvas *c2 = new TCanvas("c","",500,500);
   hTmp->Draw();

   cout << endl << "     Data" << endl;
   TGraphAsymmErrors * gdata = getRBSignal(photonMinPt,minJet,ajCut,"anaEvtSel","(1==1)","../output-data-Photon-v7_v19.root",1,isolScheme,subDPhiSide,subSShapeSide,drawCheck);
   ...
   cout << endl << "     MC" << endl;
   TGraphAsymmErrors * ghypho = getRBSignal(photonMinPt,minJet,ajCut,"offlSel&&sampleWeight>0.5","weight","../output-hy18pho50mixdj80emdj120em_v18.root",0,isolScheme,subDPhiSide,subSShapeSide,drawCheck);
   ghypho->SetMarkerSize(1.25);
   ghypho->SetMarkerStyle(kOpenSquare);
   ghypho->Draw("p same");
   
   cout << endl << "     pp" << endl;
   TGraphAsymmErrors * gpp = getRBSignal(photonMinPt,minJet,ajCut,"anaEvtSel","(1==1)","../output-data-pp2010-prod3-photon_v18.root",2,isolScheme,subDPhiSide,subSShapeSide,drawCheck);
   ...
   c2->Print(Form("%s/Photonv7_v19_RB%.0fSubDPhi%dSS%d_gamma%.0fjet%.0f_vs_Npart_Isol%d_drawChk%d.gif",outdir.Data(),ajCut*100,subDPhiSide,subSShapeSide,photonMinPt,minJet,isolScheme,drawCheck));
   c2->Print(Form("%s/Photonv7_v19_RB%.0fSubDPhi%dSS%d_gamma%.0fjet%.0f_vs_Npart_Isol%d_drawChk%d.pdf",outdir.Data(),ajCut*100,subDPhiSide,subSShapeSide,photonMinPt,minJet,isolScheme,drawCheck));
}
