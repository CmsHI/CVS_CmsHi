...
void drawVariable(int lowCent=0, int highCent=0, double lowPt=0, double highPt=0, TH1D* hvar=0, TString var="", TCut addCut="", bool doLog=false,bool onleMC=false, float maxY = 1.2, bool onlySig=false);
void drawVarAllCent (double lowPt=0, double highPt=0, TH1D* hvar=0, TString var="", TString varName="", TCut addCut="", bool doLog=false, TString subName="", bool onleMC=false , float maxY = 1.2, bool onlySig=false);
...
void mcVSdata() { 
  ...
  */
  ...
  drawVarAllCent(60,10000,hhoe, var, "H/E", "",true,"hadronicOverEm",false,2,false);
   ...
   var= "corrPt";
   drawVarAllCent(60,10000,hpt, var, "Transverse Energy", "hadronicOverEm<0.1 && sigmaIetaIeta<0.01 && (cc4+cr4+ct4PtCut20)/0.9<5",true,"pt",false,2,true);

   TH1D* heta   = new TH1D("heta",";#eta;Normalized Entries",6,-1.5,1.5);                                                 var= "eta";
   drawVarAllCent(60,10000,heta, var, "#eta", "hadronicOverEm<0.1 && sigmaIetaIeta<0.01 && (cc4+cr4+ct4PtCut20)/0.9<5",false,"eta",false,0.5,true);

   TH1D* hphi   = new TH1D("hphi",";#phi;Normalized Entries",6,-3.2,3.2);                                           
   var= "phi";
   drawVarAllCent(60,10000,hphi, var, "#phi", "hadronicOverEm<0.1 && sigmaIetaIeta<0.01 && (cc4+cr4+ct4PtCut20)/0.9<5",false,"phi",false,0.5,true);
   ...
   drawVarAllCent(60,10000,hiso, var, "#SigmaISO  ", "",false,"sumISO",false, 0.5);
   /*

   /*
   TH1D* hecal   = new TH1D("heiso",";Ecal ISO (Gev) ;Normalized Entries",30,-30,80);
   var= "cc4/.9";
   drawVarAllCent(60,10000,hecal, var, "Ecal ISO  (H/E<0.1)", "",false,"ecalIso",false, 0.8);
   
   TH1D* hhcal   = new TH1D("hhiso",";Hcal ISO (Gev) ;Normalized Entries",30,-30,80);
   var= "cr4/.9";
   drawVarAllCent(60,10000,hhcal, var, "Hcal ISO", "",false,"hcalIso",false, 0.8);

   TH1D* htrack   = new TH1D("htrack",";Track ISO (Gev) ;Normalized Entries",30,-30,80);
   var= "ct4PtCut20/.9";
   drawVarAllCent(60,10000,htrack, var, "Track ISO ", "",false,"trackIso",false, 0.8);


   TH1D* hFisher  = new TH1D("hfisher",";Fisher Discriminant ;Event Fraction",60,-4,4);
   drawVarAllCent(60,10000,hFisher, fisherVar, "Fisher Discriminant", "",false,"fisherIso",false, 0.8);   

   TH1D* hecalUc   = new TH1D("heisouc",";Uncorrected Ecal ISO (Gev) ;Normalized Entries",50,0,100);         
   var= "c4";
   drawVarAllCent(60,10000,hecalUc, var, "Uncorrected Ecal ISO ", "",false,"c4",false, 0.8);
   
   TH1D* hhcalUc   = new TH1D("hhisouc",";Uncorrected Hcal ISO (Gev) ;Normalized Entries",50,0,100);
   var= "r4";
   drawVarAllCent(60,10000,hhcalUc, var, "Uncorrected Hcal ISO ", "",false,"r4",false, 0.8);
   
   TH1D* htrackUc   = new TH1D("hhisouc",";Uncorrected Hcal ISO (Gev) ;Normalized Entries",50,0,100);
   var= "t4";
   drawVarAllCent(60,10000,htrackUc, var, "Uncorrected Track ISO ", "",false,"t4",false, 0.8);

   */

}

void drawVarAllCent (double, double highPt, TH1D* hvar, TString var,TString varName,  TCut addCut, bool doLog, TString subName,bool onleMC, float maxY, bool onlySig){
  
   TCanvas* c2 = new TCanvas(Form("c2_%s%s",var.Data(),subName.Data()),"",100+nCent_std*300,400);
   ...
   for ( int icent = 1 ; icent<=nCent_std ; icent++) {
      ...
      c2->cd(1+nCent_std-icent);
      drawVariable(lowCent,highCent,60,10000,hvar, var, addCut,doLog,onleMC, maxY,onlySig);
      if (doLog )    
	 gPad->SetLogy();
      ...
      if ( icent == 1 ) {
	 if (!onleMC) drawCMS2011(0.138,0.906,150,16);
      }
      if (icent==2) { 
	 ...
	 if (!onlySig) leg1->AddEntry(mcBkg,"MC dijet Background","fl");
	 leg1->Draw();
      }
...
void drawVariable(int lowCent, int highCent, double lowPt, double highPt, TH1D* hvar, TString var, TCut addCut, bool doLog ,bool onleMC, float maxY, bool onlySig) { 
   ...
   TString fname1 = "barrelHiForestPhotonV7.root";
   ...
   scaleInt(tempHistMC1);
   scaleInt(tempHistMC2);
   scaleInt(tempHist);
