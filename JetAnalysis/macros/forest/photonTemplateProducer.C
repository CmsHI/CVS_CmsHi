...
void photonTemplateProducer(int isoChoice = kSumIso) {
   ...
   for ( int icent = 1 ; icent<=nCent_std ; icent++) {
      ...
      fitResult fitr = doFit ( hSig[icent], hBkg[icent], hData[icent], nSig, nSigErr, 0,0.025, (icent==3),chisq,purity10);
   ...
   c1->SaveAs(Form("fittingPurity_%s.gif",getIsoLabel(isoChoice)));
...
void getTemplate(TH1D* h1, TString fname1, int isoChoice, int iTemp, int lowCent, int highCent, TCut addCut) { 
   ...
   float nEvtPho50     = 76720;
   char* fnamePho80 = "barrelHiForestPhoton_MCphoton80_28k.root";
   ...
   htemp->Reset();
   htemp->Sumw2();
   photon1->Draw2(htemp,  "sieie44",   finalCut,  weightBit);
   if ( iTemp == kData ) {
