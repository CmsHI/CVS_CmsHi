...
void photonTemplateProducer(int ppHI = kHI, int isoChoice = kSumIso, int isoCut = -100, bool onlygjEvents=true, float specialSbCut=10, float mcSigShift=0) {
  ...
  TCanvas* c1[5];
   ...
   if ( ppHI == kHI) makeMultiPanelCanvas(c2,nCent_std/2,2,0.0,0.0,0.2,0.15,0.02);
   ...
   for (int icent = 1; icent <=nCent; icent++) {
     ...
     heffDphi[icent] = new TH1D(Form("heff_icent%d_Dphi",icent),";#Delta#phi of pair;Efficiency",10,0,3.141592);
   ...
   for (int icent = 1; icent <=nCent_std; icent++) {
     ...
     hPurity[icent]->GetFunction("pol1")->SetLineColor(color[icent]);
