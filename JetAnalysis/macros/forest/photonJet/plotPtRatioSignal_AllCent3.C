...
public:
   ...
   name(n),
   sel(s),
   rSigAll(n+"SignalAll","jetEt/photonEt",s&&"acos(cos(photonPhi-jetPhi))>2.0944 && sigmaIetaIeta<0.01",w),
   rBkgDPhi(n+"BkgDPhi","jetEt/photonEt",s&&"acos(cos(photonPhi-jetPhi))>0.7 && acos(cos(photonPhi-jetPhi))<3.14159/2. && sigmaIetaIeta<0.01",w),
   rBkgSShape(n+"BkgSShape","jetEt/photonEt",s&&"acos(cos(photonPhi-jetPhi))>2.0944 && sigmaIetaIeta>0.011",w),
   useWeight(w),
   subDPhiSide(true),
   subSShapeSide(true) {
      t = tree;
   ...
   void MakeHistograms(float fracPhotonBkg) {
      rSigAll.Init(t,20,0.001,1.999,1.);
      if (subDPhiSide) {
         ...
         rBkgDPhi.Init(t,20,0.001,1.999,fracDPhiBkg);
      ...
      if (subSShapeSide) {
         ...
         rBkgSShape.Init(t,20,0.001,1.999,fracPhotonBkg);
      }
...
//---------------------------------------------------------------------
TH1D * plotPtRatio(int cbin, int isolScheme,
                   TString infname,
                   ...
                   bool doCheck=false)
{
   ...
   SignalCorrector anaRatio(nt,name,cut,useWeight);
   
   // analyze tree
   if (dataType==0) {
      anaRatio.subDPhiSide = false;
      anaRatio.subSShapeSide = false;
      anaRatio.rSigAll.cut = cut;
      anaRatio.MakeHistograms(0);
   } else if (dataType==1) {
      anaRatio.subDPhiSide = true;
      anaRatio.subSShapeSide = true;
      if (!isData) anaRatio.subSShapeSide = false; // todo: get purity for mc sample
      anaRatio.MakeHistograms(1-photonPurity);
   }
   
   // histogram style
   if (isData) {
      anaRatio.hSubtracted->SetLineColor(kRed);
      anaRatio.hSubtracted->SetMarkerColor(kRed);
      anaRatio.hSubtracted->SetMarkerStyle(20);
   } else {
      if (dataType==0) {
         anaRatio.hSubtracted->SetLineColor(kBlue);
         anaRatio.hSubtracted->SetFillColor(kAzure-8);
         anaRatio.hSubtracted->SetFillStyle(3005);
         anaRatio.hSubtracted->SetMarkerStyle(0);
      } else if (dataType==1) {   
         anaRatio.hSubtracted->SetLineColor(kBlue);
         anaRatio.hSubtracted->SetFillColor(kAzure-8);
         anaRatio.hSubtracted->SetFillStyle(3005);
         ...
         anaRatio.hSubtracted->SetLineColor(kBlue);
         anaRatio.hSubtracted->SetFillColor(kAzure-8);
         anaRatio.hSubtracted->SetFillStyle(3004);
         anaRatio.hSubtracted->SetStats(0);
         anaRatio.hSubtracted->SetLineStyle(2);
         anaRatio.hSubtracted->SetMarkerStyle(0);
      }
   }
   anaRatio.hSubtracted->Draw(opt);
   if (doCheck) {
      if (anaRatio.subDPhiSide) {
         anaRatio.rBkgDPhi.hScaled->SetMarkerStyle(kOpenCircle);
         anaRatio.rBkgDPhi.hScaled->SetMarkerColor(kGreen+2);
         anaRatio.rBkgDPhi.hScaled->Draw("sameE");
      }
      if (anaRatio.subSShapeSide) {
         anaRatio.rBkgSShape.hScaled->SetMarkerStyle(kOpenCircle);
         anaRatio.rBkgSShape.hScaled->SetMarkerColor(kViolet);
         anaRatio.rBkgSShape.hScaled->Draw("sameE");
      }
      anaRatio.rSigAll.hScaled->SetLineStyle(2);
      anaRatio.rSigAll.hScaled->Draw("same hist");
      // Draw count
      float nPhotonJet = anaRatio.rSigAll.h->GetEntries();
      if (anaRatio.subDPhiSide) nPhotonJet -= anaRatio.rBkgDPhi.h->GetEntries()*(3.14159-2.0944)/(3.14159/2.-0.7);
      if (anaRatio.subSShapeSide) nPhotonJet -= anaRatio.rBkgSShape.h->GetEntries();
      float lx = 0.1;
      if (cbin<2) lx=-0.1;
      TLegend *t3=new TLegend(lx,0.7,0.6,0.85);
      if (cbin==0) t3->AddEntry(anaRatio.rSigAll.h,nameIsol,"");
      t3->AddEntry(anaRatio.rSigAll.h,Form("%.0f #gamma-jets",nPhotonJet),"");
      t3->AddEntry(anaRatio.rSigAll.h,Form("#gamma purity %.2f",photonPurity),"");
      t3->SetFillColor(0);
      t3->SetBorderSize(0);
      t3->SetFillStyle(0);
      t3->SetTextFont(63);
   ...
   return anaRatio.hSubtracted;
}

//---------------------------------------------------------------------
void plotPtRatioSignal_AllCent3(
                                ...
                                )
{
   ...
   hFrame->Draw();
   //plotPtRatio(2,-1,"../output-hypho50gen_v4.root",true,false,0,"samehist",false);
   plotPtRatio(2,isolScheme,"../output-hypho50v2_50kyongsun_v11.root",true,false,1,"samehistE",0);
   plotPtRatio(2,isolScheme,"../output-data-Photon-v2_v11.root",false,true,1,"sameE",1);
   drawText("30-100%",0.83,0.35);
   drawText("(a)",0.25,0.885);
   TLatex *cms = new TLatex(1.1,0.36,"CMS Preliminary");
   cms->SetTextFont(63);
   cms->SetTextSize(17);
   cms->Draw();
   TLatex *lumi = new TLatex(1.1,0.23,"#intL dt = 84 #mub^{-1}");
   lumi->SetTextFont(63);
   lumi->SetTextSize(17);
   lumi->Draw();
   TLatex *cme = new TLatex(1.1,0.30,"#sqrt{s}_{_{NN}}=2.76 TeV");
   cme->SetTextFont(63);
   cme->SetTextSize(17);
   cme->Draw();
   TLatex *jetf_PbPb;
   jetf_PbPb = new TLatex(1.3,0.14,"Ak PF, R=0.3");
   ...
   hFrame->Draw();
   //plotPtRatio(1,-1,"../output-hypho50gen_v4.root",true,false,0,"samehist",false);
   plotPtRatio(1,isolScheme,"../output-hypho50v2_50kyongsun_v11.root",true,false,1,"samehistE",0);
   plotPtRatio(1,isolScheme,"../output-data-Photon-v2_v11.root",false,true,1,"sameE",1);
   drawText("10-30%",0.8,0.35);
   ...
   hFrame->Draw();
   //plotPtRatio(0,-1,"../output-hypho50gen_v4.root",true,false,0,"samehist",false);
   plotPtRatio(0,isolScheme,"../output-hypho50v2_50kyongsun_v11.root",true,false,1,"samehistE",0);
   plotPtRatio(0,isolScheme,"../output-data-Photon-v2_v11.root",false,true,1,"sameE",1);
   drawText("0-10%",0.8,0.35);
   ...
   c1->Print(Form("./fig/12.13newfisher/photon60v2_v11_jet30_ptratio_all_cent_subAll_Isol%d.gif",isolScheme));
   c1->Print(Form("./fig/12.13newfisher/photon60v2_v11_jet30_ptratio_all_cent_subAll_Isol%d.pdf",isolScheme));   

   // save histograms
//   fout->Write();
//   TCanvas * ctest = new TCanvas("ctest","",500,500);
//   hFrame->Draw();
//   plotPtRatio(0,"../output-data-Photon-v2d1204_v9.root",false,1,"sameE",1);
//   plotPtRatio(0,"../output-data-Photon-v2_v11.root",false,1,"sameE",1);
}
