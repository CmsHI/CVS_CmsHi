...
void plotRBDphi(  double ajCut= 3.025,
	      double ajCut2 = 3.025,
		 TString infname = "output-40-HI.root",
...
*/
  const int nBin = 7;
  double m[nBin+1] = {-1.5,-0.5,3.5,7.5,11.5,20.5,28.5,40.5};
  double npart[nBin] = {2,358.623,232.909,42.026,8.748};
  double npart2[nBin] = {2,358.623,232.909,42.026,8.748};
  ...
  Float_t bin=0;
  Float_t et1=0;
  nt->SetBranchAddress("cBin",&bin);
  nt->SetBranchAddress("photonEt",&et1);
  ...
  nt->Draw("cBin>>h",Form("jetEt>40&&acos(cos(photonPhi-jetPhi))>%f&&%s",ajCut,cut1.Data()));
  nt->Draw("cBin>>hCut",Form("%s",cut1.Data()));
  TGraphAsymmErrors *g = calcEff(hCut,h,npart);
  g->SetMarkerSize(1.25);

  cout <<cut2.Data()<<endl;
  nt->Draw("cBin>>h2",Form("jetEt>40&&acos(cos(photonPhi-jetPhi))>%f&&%s",ajCut2,cut2.Data()));
  nt->Draw("cBin>>h2Cut",Form("%s",cut2.Data()));
  TGraphAsymmErrors *g2 = calcEff(h2Cut,h2,npart2);
  g2->SetMarkerSize(1.25);


  ntPythia->Draw("cBin>>h",Form("jetEt>40&&acos(cos(photonPhi-jetPhi))>%f&&%s",ajCut2,cut2.Data()));
