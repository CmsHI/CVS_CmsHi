...
public:
   ...
   name(n),
   sel(s),
   rSigAll(n+"SignalAll","acos(cos(photonPhi-jetPhi))",s&&"acos(cos(photonPhi-jetPhi))>0.7 && sigmaIetaIeta<0.01",w),
   rBkgDPhi(n+"BkgDPhi","acos(cos(photonPhi-jetPhi))",s&&"acos(cos(photonPhi-jetPhi))>0.7 && acos(cos(photonPhi-jetPhi))<3.14159/2. && sigmaIetaIeta<0.01",w),
   rBkgSShape(n+"BkgSShape","acos(cos(photonPhi-jetPhi))",s&&"acos(cos(photonPhi-jetPhi))>0.7 && sigmaIetaIeta>0.011",w),
   useWeight(w),
   subDPhiSide(true),
   subSShapeSide(true) {
      t = tree;
   ...
   void MakeHistograms(float fracPhotonBkg) {
      rSigAll.Init(t,20,0.0001,3.1415926,1.);
      if (subDPhiSide) {
         ...
         rBkgDPhi.Init(t,20,0.0001,3.1415926,fracDPhiBkg);
      ...
      if (subSShapeSide) {
         ...
         rBkgSShape.Init(t,20,0.0001,3.1415926,fracPhotonBkg);
      }
                                ...
                                )
{
   ...
   plotBalance(2,isolScheme,"../output-hypho50v2_50kyongsun_v11.root",true,false,1,"samehistE",0);
   plotBalance(2,isolScheme,"../output-data-Photon-v2_v11.root",false,true,1,"sameE",1);
   ...
   plotBalance(1,isolScheme,"../output-hypho50v2_50kyongsun_v11.root",true,false,1,"samehistE",0);
   plotBalance(1,isolScheme,"../output-data-Photon-v2_v11.root",false,true,1,"sameE",1);
   ...
   plotBalance(0,isolScheme,"../output-hypho50v2_50kyongsun_v11.root",true,false,1,"samehistE",0);
   plotBalance(0,isolScheme,"../output-data-Photon-v2_v11.root",false,true,1,"sameE",1);
   ...
   c1->Print(Form("./fig/12.13newfisher/photon60v2_v11_mcv2yongsun_jet30_dphi_all_cent_subAll_Isol%d.gif",isolScheme));
   c1->Print(Form("./fig/12.13newfisher/photon60v2_v11_mcv2yongsun_jet30_dphi_all_cent_subAll_Isol%d.pdf",isolScheme));   
   ...
   //plotBalance(-1,isolScheme,"../output-hypho50v2_50kyongsun_v11.root",true,false,1,"samehist",1);
   //plotBalance(-1,isolScheme,"../output-data-Photon-v2_v11.root",false,true,1,"sameE",1);
