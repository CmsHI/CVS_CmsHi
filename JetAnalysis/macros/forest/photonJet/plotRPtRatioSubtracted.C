...
)
{
   ...
   if (dataType==2) anaDen.subSShapeSideDPhiSide = false; // no dphi subtraction for photon sideband
                      ...
                      )
{
   ...
   hTmp->SetYTitle("<E_{T,Jet}/E_{T,#gamma}> (GeV)");
   hTmp->GetXaxis()->CenterTitle();
   hTmp->GetYaxis()->CenterTitle();
   hTmp->GetYaxis()->SetTitleOffset(1.4);
   hTmp->GetYaxis()->SetTitleSize(0.05);
   float ymax=2; // 35, 2, 0.4
   ...
   TLegend *leg2=new TLegend(0.58,0.49,0.90,0.67);
