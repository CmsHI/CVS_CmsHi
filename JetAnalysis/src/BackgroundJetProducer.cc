#ifndef RecoHIJets_JetProducers_plugins_BackgroundJetProducer_h
#define RecoHIJets_JetProducers_plugins_BackgroundJetProducer_h

#include "MyVirtualJetProducer.h"

class BackgroundJetProducer : public MyVirtualJetProducer
{
...
CLHEP::HepRandomEngine* bkgRandomEngine;
...
void BackgroundJetProducer::writeBkgJets( edm::Event & iEvent, edm::EventSetup const& iSetup )
{
   ...
   for(int ijet = 0; ijet < nFill_; ++ijet){
      phiRandom[ijet] = 2*pi*bkgRandomEngine->flat() - pi;
      etaRandom[ijet] = 2*etaMax_*bkgRandomEngine->flat() - etaMax_;
      et[ijet] = 0;
      pileUp[ijet] = 0;
   }
